...
- Release (plugin + server image): `npm version patch -w plugin -w server`
  (plugin's script syncs manifest.json/versions.json; server version surfaces
  at /healthz) → commit → `git tag <version>` (bare semver, no `v`, must equal
  both versions) → `git push && git push --tags`.
