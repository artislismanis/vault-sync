...
| Version history | Keep every revision by default; retention/pruning policy configurable per vault; restore any version from plugin UI |
| Selective sync | Toggle by category (images, audio, video, PDF, other); max-file-size cap |
...
- `.obsidian` settings sync: appearance, hotkeys, core + community plugin
  configs, optionally plugin code. Desktop-focused; per-device opt-out because
  mobile wants different plugins. (Owner wants "a go" at this early — first
  candidate after MVP.)
...
- Multi-user / shared vaults, collaboration
- Headless CLI sync client
- Server-assisted housekeeping that works on ciphertext (orphan GC, quota)

## Explicit non-goals (MVP)

- No multi-user auth, no sharing, no server-side rendering/search of content
